 ...
 */
var src = '';
function showAddShoseWin(){
    getCombox("add");
    $("#addWin").window("open");// 打开指定的window， open表示打开，close表示关闭
}
function showEditShoseWin(){
...
function showEditShoseWin1(index){
...
function removeShose1(index){
    var row = $('#list').datagrid('getData').rows[index];
    if(row){
        $.messager.confirm('提示', '确定要删除吗?', function(r){
            if (r){
                $.get("/shose/deleteById?ids=" + row.id,
                    function(data){
                        $("#list").datagrid("reload"); // 重新加载指定数据网格数据
                    },"json");
            }
...
function formatterOpt(value,row,index){
    return "<a href = 'javascript:;'onclick = 'showEditShoseWin1("+index+")'>编辑</a>&nbsp&nbsp"+
        "<a href = 'javascript:;' onclick = 'removeShose1("+index+")'>删除</a>";
}
// 显示品牌，材质，机芯，等描述
...
function queryByTitle(){
    ...
    var title = $("#title").val();
    if (title != "" || brand != "") {
        $.get("/shose/query_pager?page=1&rows=20&brand=" + brand + "&title="+title,
            function(data) {
...
function queryByPage(){
    $.get("/shose/query_pager?page=1&rows=20",
        function(data){
